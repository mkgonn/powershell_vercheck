...
| Az | 8.0.0 | **13.5.0** | Az.Accounts 4.x で動作する最高版 |

| Az.Accounts | 4.0.2 | **4.2.0** | 5.x は PnP.PowerShell と競合するため不可 |
...
| Az.KeyVault | 4.5.0 | **6.3.1** | Az 13.5.0 同梱版 |
...
| Az.Websites | 2.11.2 | **3.4.1** | Az 13.5.0 同梱版 |
...
| PnP.PowerShell | 1.11.0 | **1.12.0** | PS 5.1 対応の最終版（2.x 以降は PS7+ 必須） |
...
   - 既知の破壊的変更例: `Resolve-Error` エイリアス削除（Az 13.0.0）
...
2. **Microsoft.Graph 2.x への将来的な移行**

   - コマンドレット名・パラメータ・戻り値に破壊的変更あり

   - スクリプト改修が必要

   - 今回は 1.28.0（1.x 最終版）を推奨、2.x 移行は別途計画
...
RUN powershell Install-Module Az -RequiredVersion 13.5.0 -Force

RUN powershell Install-Module Az.Accounts -RequiredVersion 4.2.0 -Force -AllowClobber
...
Connect-PnPOnline -Url [https://tenant.sharepoint.com](https://tenant.sharepoint.com) -Interactive

```

### 2. PnP.PowerShell 3.x の存在

PnP.PowerShell は既に **3.1.0**（PS 7.4.6 必須）がリリースされている。報告書では「2.x 以降は PS7+ 必須」と記載しているが、**3.x が最新メジャーライン** であることを認識しておくべき。将来の PS 7 移行計画では 3.x を検討対象とすること。
...
`Az 13.5.0` をインストールすると依存関係により **Az.Accounts の最新互換版が自動インストールされる**可能性がある。その後に `Az.Accounts 4.2.0` を `-AllowClobber` で上書きする方式は動作するが、**インストール順序を逆にする方が安全**:

```dockerfile

# 先に Az.Accounts を固定してからAz をインストール
...
MSAL.PS は **2021年11月を最後に更新が停止** しており、実質的にメンテナンスされていない。依存している場合、将来的に認証フローの非互換（Azure AD → Entra ID 移行等）が発生するリスクがある。代替として `Microsoft.Identity.Client`（MSAL.NET）の直接利用や、Az.Accounts 組み込みの認証機能への移行を検討すべき。

### 6. Microsoft.Graph 1.28.0 の EOL リスク

Microsoft.Graph 1.x は **2023年6月が最終リリース** であり、Microsoft は 2.x への移行を推奨している。セキュリティ修正も 2.x のみに提供される可能性が高い。今回は 1.28.0 を採用するが、**早期に 2.x 移行計画を策定すること**を強く推奨する。2.x は PS 5.1 に対応しているため、PS バージョンの制約にはならない。
...
3. Az.Compute / Az.Storage / Az.Network / Az.Sql の **Breaking Changes ドキュメントを確認**し、使用コマンドレットへの影響を調査

4. 検証通過後、expected_modules.csv と Dockerfile を更新（**インストール順序に注意**）

5. Microsoft.Graph 2.x 移行は別途計画として切り出し（**PS 5.1 対応済みのため技術的障壁は低い**）
