...
| Microsoft.Online.SharePoint.PowerShell        | 16.0.8029.0 | 非ネイティブ     | 非対応           | BLOCK・要代替   |
...
### 5. Microsoft.Online.SharePoint.PowerShell 16.0 — 最大の問題モジュール

- **PS7**: **ネイティブ非対応**。`Import-Module -UseWindowsPowerShell` で動作可能だが、これはWindows上でのみ使える互換レイヤー
- **Linux**: **完全に非対応**。UseWindowsPowerShell互換レイヤーはWindowsのみ。Azure Cloud Shell（Linux）でも非対応と公式Q&Aで確認
- **技術的理由**: .NET Framework 4.x依存、WinRM依存
